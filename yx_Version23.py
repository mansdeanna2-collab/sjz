...
# ==================== 用户配置 ====================
ENGINE_PATH   = r"C:\Users\Administrator\PyCharmMiscProject\runs\detect\my_experiment39\weights\best1_320.engine"
IMG_SIZE, CENTER_REGION, NUM_CLASSES = 320, 320, 4

CONF_KEEP, CONF_NEW = 0.30, 0.55
...
DRAW_CROSSHAIR, DRAW_STATS, DRAW_FOV_CIRCLE = True, True, True
STATS_UPDATE_MS = 100

# ==================== 瞄准配置 (v21) ====================
AIM_ENABLE = True
AIM_HOTKEYS  = ["RBUTTON"]        # 开镜吸附键
AIM_FIRE_KEY = "LBUTTON"

# ★★ 开火即吸附 (核心新功能) ★★
AIM_FIRE_TRIGGERS_AIM = True      # 开火时自动吸附 (哪怕没按右键)
AIM_FIRE_ONLY_FOV_MULT = 0.75     # 仅开火时 FOV 收窄倍率 (避免误锁远处)
AIM_FIRE_ONLY_STRENGTH = 0.80     # 仅开火时吸附强度倍率 (0.6~1.0)

AIM_CLASSES = [0]
AIM_OFFSETS = {0: 0.15}
AIM_DEFAULT_OFFSET = 0.15
AIM_FOV_RADIUS = 180

# P 控制
AIM_KP_X = 0.12
AIM_KP_Y = 0.12
AIM_FIRE_KP_X_MULT = 1.15
AIM_FIRE_KP_Y_MULT = 1.80

# 快速锁定
AIM_ACQUIRE_BOOST_MS = 150
AIM_ACQUIRE_KP_MULT  = 1.8
AIM_ACQUIRE_CAP_MULT = 1.6

# 输出平滑
AIM_OUTPUT_SMOOTH = 0.30

# 瞄准点平滑
AIM_POINT_SMOOTH_NEW  = 0.55
AIM_POINT_SMOOTH_FIRE = 0.40

# 锁定位置平滑
AIM_LOCK_SMOOTH_ALPHA = 0.65

# 步长
AIM_STEP_CAP_NEAR = 4
AIM_STEP_CAP_FAR  = 14
AIM_STEP_CAP_FIRE_Y = 18
AIM_NEAR_DIST = 25
AIM_FAR_DIST  = 100
...
AIM_BRAKE_FACTOR = 0.30
AIM_BRAKE_FIRE_X_FACTOR = 0.20

# 压枪 (步枪)
AIM_ANTI_RECOIL = True
AIM_RECOIL_Y_BASE = 0.8
AIM_RECOIL_Y_RAMP = 0.006
AIM_RECOIL_Y_MAX  = 2.0
AIM_RECOIL_X      = 0.0
AIM_RECOIL_START_DELAY_MS = 80

# ★ 目标评分 (多目标选优)
AIM_SCORE_DIST_WEIGHT = 1.0       # 距离权重 (px)
AIM_SCORE_SIZE_BONUS  = 80        # 大目标加成上限 (px 等效)
AIM_SCORE_CENTER_BONUS = 50       # 靠近十字准心中心额外奖励

# 位置锁
AIM_POS_LOCK = True
AIM_LOCK_BASE_TOL    = 150
AIM_LOCK_MOVE_FACTOR = 9.0
AIM_LOCK_FIRE_BONUS  = 1.7
AIM_LOCK_STICKY_MS   = 600
AIM_LOCK_FIRE_NO_SWITCH = True
AIM_LOCK_SIZE_WEIGHT = 0.15

# 目标速度
AIM_TARGET_VEL_TRACK  = True
AIM_TARGET_VEL_TOL    = 0.10
AIM_TARGET_VEL_ALPHA  = 0.40
AIM_TARGET_PRED_MS    = 30
AIM_TARGET_VEL_MAX    = 300
AIM_PRED_DISABLE_ON_FIRE = True

# ★ 目标滑行 (丢失瞬间保持方向)
AIM_COAST_MS = 80                 # 丢目标 N ms 内保持上帧运动
AIM_COAST_DECAY = 0.75            # 每帧衰减
...
class Aimbot:
    ...
    def _pick_target(self, boxes, confs, clss, now, firing, fov_radius):
        ...
        for b, c, k in zip(boxes, confs, clss):
            k_i = int(k)
            if k_i not in AIM_CLASSES: continue
            ...
            cands.append((d2, b, k_i, scx, scy, sw, sh, ax, ay))
    ...
    def _recoil_y(self):
        base = AIM_RECOIL_Y_BASE
        extra = self._fire_frames * AIM_RECOIL_Y_RAMP
        return min(base + extra, AIM_RECOIL_Y_MAX)

    def tick(self, boxes, confs, clss, ids):
        self._probe(clss)
        calibrating = is_any_key_down(AIM_CALIB_KEYS)
        ads_pressed = is_any_key_down(AIM_HOTKEYS)   # 开镜键
        ...
        if pick is None:
            ...
            if now < self._coast_until and (abs(self._coast_mx) > 0 or abs(self._coast_my) > 0):
                ...
                self._coast_mx = cmx; self._coast_my = cmy
                self._maybe_send(now, cmx, cmy, "coast")
            self.aim_point = None
            self.locked_box_screen = None
            return False
        ...
        mx = (1.0 - AIM_OUTPUT_SMOOTH) * mx_raw + AIM_OUTPUT_SMOOTH * self._last_mx_raw
        my = (1.0 - AIM_OUTPUT_SMOOTH) * my_raw + AIM_OUTPUT_SMOOTH * self._last_my_raw
        ...
        if dist <= AIM_NEAR_DIST:
            cap = AIM_STEP_CAP_NEAR
        elif dist >= AIM_FAR_DIST:
            cap = AIM_STEP_CAP_FAR
        else:
            t = (dist - AIM_NEAR_DIST) / (AIM_FAR_DIST - AIM_NEAR_DIST)
            cap = AIM_STEP_CAP_NEAR + (AIM_STEP_CAP_FAR - AIM_STEP_CAP_NEAR) * t
        cap *= boost_cap
        cap_y = max(cap, AIM_STEP_CAP_FIRE_Y) if firing else cap

        mx = max(-cap,   min(cap,   mx))
        ...
        # 更新滑行状态
        self._coast_mx = mx; self._coast_my = my
...
class Overlay(threading.Thread):
    ...
    def _on_paint(self,hwnd):
        ...
        try:
            ...
            with self._lock:
                boxes=self._boxes; confs=self._confs
                clss=self._clss; ids=self._ids
                stats=self._stats; aim_active=self._aim_active
                aim_point=self._aim_point; locked_box=self._locked_box
                self._dirty=False
            ...
            top_prio=AIM_CLASSES[0] if AIM_CLASSES else 0

            if boxes is not None:
                for b,c,k,tid in zip(boxes,confs,clss,ids):
                    k_i=int(k)
                    if k_i==top_prio: pen=self._pen_prio; rgb=win32api.RGB(0,255,0)
                    else: pen=self._pen_other; rgb=win32api.RGB(255,140,0)
                    op=win32gui.SelectObject(mdc,pen); ob=win32gui.SelectObject(mdc,self._null_brush)
                    win32gui.Rectangle(mdc,int(b[0]),int(b[1]),int(b[2]),int(b[3]))
                    win32gui.SelectObject(mdc,op); win32gui.SelectObject(mdc,ob)
                    label=f"#{int(tid)} {self.names.get(str(k_i),str(k_i))} {c:.2f}"
                    win32gui.SetTextColor(mdc,rgb)
                    win32gui.DrawText(mdc,label,-1,(int(b[0]),max(int(b[1])-16,0),int(b[0])+220,max(int(b[1])-16,0)+18),tf)

            if locked_box is not None:
                lx1=locked_box[0]-self.x; ly1=locked_box[1]-self.y
                lx2=locked_box[2]-self.x; ly2=locked_box[3]-self.y
                op=win32gui.SelectObject(mdc,self._pen_lock)
                ob=win32gui.SelectObject(mdc,self._null_brush)
                try: win32gui.Rectangle(mdc,int(lx1),int(ly1),int(lx2),int(ly2))
                finally:
                    win32gui.SelectObject(mdc,op); win32gui.SelectObject(mdc,ob)
            ...
            if DRAW_FOV_CIRCLE and aim_active and AIM_FOV_RADIUS>0 and self._pen_fov:
                cx,cy=self.w//2,self.h//2
                op=win32gui.SelectObject(mdc,self._pen_fov)
                ob=win32gui.SelectObject(mdc,self._null_brush)
                try: win32gui.Ellipse(mdc,cx-AIM_FOV_RADIUS,cy-AIM_FOV_RADIUS,cx+AIM_FOV_RADIUS,cy+AIM_FOV_RADIUS)
                finally:
                    win32gui.SelectObject(mdc,op); win32gui.SelectObject(mdc,ob)

            if aim_point is not None:
                apx=int(aim_point[0]-self.x); apy=int(aim_point[1]-self.y)
                rb=win32gui.CreateSolidBrush(win32api.RGB(255,0,0))
                win32gui.FillRect(mdc,(apx-6,apy-1,apx+6,apy+1),rb)
                win32gui.FillRect(mdc,(apx-1,apy-6,apx+1,apy+6),rb)
                self._sd(rb)
                op=win32gui.SelectObject(mdc,self._pen_aim)
                ob=win32gui.SelectObject(mdc,self._null_brush)
                try: win32gui.Ellipse(mdc,apx-10,apy-10,apx+10,apy+10)
                finally:
                    win32gui.SelectObject(mdc,op); win32gui.SelectObject(mdc,ob)
...
def main():
    ...
    print(f"[aim] 瞄点平滑={AIM_POINT_SMOOTH_NEW}(开火{AIM_POINT_SMOOTH_FIRE}) 锁定平滑={AIM_LOCK_SMOOTH_ALPHA}")
    print(f"[aim] 压枪={AIM_ANTI_RECOIL} Y={AIM_RECOIL_Y_BASE}+{AIM_RECOIL_Y_RAMP}*n max={AIM_RECOIL_Y_MAX}")
    print(f"[aim] 锁定 tol={AIM_LOCK_BASE_TOL} sticky={AIM_LOCK_STICKY_MS}ms 滑行={AIM_COAST_MS}ms")
    print(f"[aim] F2 探测, F3 校准")
